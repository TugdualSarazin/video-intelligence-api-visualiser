...
Vue.component('object-tracking-viz', {
    ...
    computed: {
        object_tracks: function () {
            ...
            if (!this.json_data[0] || !this.json_data[0].entity)
                return []
...
class Object_Track {
    constructor(json_data, video_height, video_width) {
        ...
        json_data.frames.forEach(frame => {
            const new_frame = {
                'box': {
                    'x': (frame.box_video.left || 0) * video_width,
                    'y': (frame.box_video.top || 0) * video_height,
                    'width': ((frame.box_video.right || 0) - (frame.box_video.left || 0)) * video_width,
                    'height': ((frame.box_video.bottom || 0) - (frame.box_video.top || 0)) * video_height
                },
